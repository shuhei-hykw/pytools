...
def draw(scintillator, pmt, light_guide, label_position, scale_height,
         full_width, wave=None, beam=None, with_dash=False):
  ...
  if settings.target == 'FBH':
    db.draw_square([(a4size[0]-total_width)/2, y],
                   total_width, pmt['height'], 0.3, 0.4)
    db.draw_square([(a4size[0]-n*pmt['width'])/2, y+pmt['height']],
                   n*pmt['width'], pmt['height'], 0.3)
    db.draw_text([(a4size[0]-total_width)/2-22, y], 'Fiber fixing frame')
    db.draw_arrow([(a4size[0]-total_width)/2-2, y+pmt['height']/2], 4, 0, 3)
  ...
  db.draw_arrow([a4size[0]/2 - len(light_guide['segments'])/2*pmt['width']
                 + light_guide['label'][0]
                 + (11.5 if settings.target == 'FBH' else 19),
                 ycenter + light_guide['height'] + light_guide['label'][1]],
                4, -2, 3)
  db.draw_text([a4size[0]/2 - len(light_guide['segments'])/2*pmt['width']
  ...
  if with_dash:
    ...
    print('[] 0 setdash')
  '''top view'''
  ycenter = (30 if settings.target == 'FBH' else
  ...
  for i in range(n):
    ...
    if ((settings.target == 'TOF' and (abs(i -n/2+0.5) < 2)) or
        (settings.target == 'SCH' and (i == n/2 or i == n/2-1))):
      if i == n/2:
    ...
    if settings.target == 'FBH' or settings.target == 'SCH':
      db.draw_circle([x + scintillator['widths'][i]/2,
                      ycenter + scintillator['zdiff']*(1-i%2) + 0.5*settings.scale],
                     0.5*settings.scale)
      if i == 4:
        db.draw_line_with_scale([x+scintillator['widths'][i]/2-0.5*settings.scale,
...
def draw_fbh(scale=1.2):
  ...
  scintillator['label'] = [-w/2+2*scale, 4]
