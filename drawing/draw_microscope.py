...
def draw():
  ...
  db.draw_text([x+w/2, y+h*0.3-1.5], 'Haldenhain MT??? (z)')
  ''' stepping motor '''
  y += 1.75*h
  db.draw_square([x, y], w, h)
  ...
  ''' Mortoer controll board '''
  y += 2*h
  db.draw_square([x, y], w, h/2)
  db.draw_text([x, y+h*0.5+3], 'Mortoer controll board', centering=False)
  ...
  db.draw_arrow([x-10, y+0.5*h], 10+w/2, 0, 3, msize=2)
  db.draw_arrow([x+w/2, y+h/2], 0, 1.45*h, 2, msize=2)
  # db.draw_square([x, y+0.*h], w, h*1)
  db.draw_text([x+w/2, y+0.3*h-1.5], 'Interpolate/digitize')
  ...
  db.draw_arrow([x+w, y+0.5*h], 10, 0, 0, msize=2)
  ...
  db.draw_arrow([x-10, y+0.5*h], 10, 0, 1, msize=2)
  db.draw_arrow([x+w, y+0.5*h], 10, 0, 1, msize=2)
  db.draw_arrow([x-10, y+1.75*h], 20+w, 0, 2, msize=2)
  db.draw_arrow([x+w*0.2, y+0.8*h], 0, 0.95*h, 1, msize=2)
  db.draw_text([x+w*0.6, y+(0.8+0.95/2)*h-1.5], 'synchronize')
  db.draw_text([x+w+12, y+0.5*h-1.5], 'on-board USB', centering=False)
