...
def draw():
  ...
  db.draw_arrow([xlens-h_all/tan, ylens-h_all-20], h_all/tan, h_all, 1, lw=0.5, msize=2)
  ...
  db.draw_text([xlens-h_all/tan+6, y+4], 'track', centering=False)
  ...
  ''' micro image '''
  xoffset = 40
  db.draw_polygon([xlens+wimage/2, ylens-22], [[xoffset-wimage*1.5, -0],
                                             [0, 4],
                                             [wimage2*1.5, 0],
                                             [0, -himage2*8],
                                             [-wimage2*1.5, 0],
                                             [0, himage2*5]], fill_color=1)
  # db.draw_text([xlens+xoffset-wimage+wimage2*0.75, ylens-20+3.5],
  #              'micro track')
  n = 6
  for i in range(n+4):
    ...
    if abs(i-(n-0.5)/2.) <= 1:
      ...
      if i == n/2:
        db.draw_circle([ximage2+xoffset+wimage2*3/4, yimage2+himage2], 0.2, fc=0)
    else:
      db.draw_square([ximage2+xoffset, yimage2], wimage2, himage2, fill_color=-1)
  for i in range(3):
    ximage -= himage*10/tan
    yimage -= himage*10
    db.draw_square([ximage+himage*10/tan/2, yimage+himage*10], wimage, himage*10, fill_color=-1)
  db.draw_arrow([xlens+xoffset-himage2*6/tan+wimage*3/4, ylens-20-himage2*6],
